...
export const COPY = {
  ...
  needs: {
    en: {
      ...
      h2: "How much will retirement cost you?",
      body: "Think of it like a monthly bill that never stops. Add up what you must spend every month — then multiply by how many years you'll live after retiring.",
      monthly: "MONTHLY IN RETIREMENT",
      ikigaiLabel: "TOTAL YOU NEED",
      ref: (g: GeoConfig) => `Most people in ${g.name} need around ${fmtC(g.retireRef, "th")} for a comfortable retirement`,
      yearsLabel: (y: number) => `${y} years of retirement`,
      livingL: "Food, home, basics",
      healthL: "Extra healthcare",
      debtL: "Mortgage or debt",
      totalMo: "TOTAL PER MONTH",
      totalYr: "PER YEAR",
      totalAll: "FOR ALL YOUR RETIREMENT",
      simpleMath: (mo: string, yr: string, yrs: number, tot: string) =>
        `${mo} per month × 12 months = ${yr} per year. ${yr} × ${yrs} years = ${tot}. That's your number.`,
    },
    th: {
      overline: "สิ่งที่คุณต้องการ",
      h2: "เกษียณต้องใช้เงินเท่าไหร่?",
      body: "คิดเหมือนบิลรายเดือนที่ไม่มีวันหยุด รวมทุกอย่างที่ต้องจ่าย — แล้วคูณด้วยปีที่เหลืออยู่หลังเกษียณ",
      monthly: "รายจ่ายต่อเดือนยามเกษียณ",
      ikigaiLabel: "เงินที่ต้องมีทั้งหมด",
      ref: (g: GeoConfig) => `คนส่วนใหญ่ใน${g.name}ต้องการประมาณ ${fmtC(g.retireRef, "th")} เพื่อเกษียณแบบสบาย`,
      yearsLabel: (y: number) => `เกษียณ ${y} ปี`,
      livingL: "อาหาร ที่อยู่ พื้นฐาน",
      healthL: "ค่ารักษาเพิ่มเติม",
      debtL: "ผ่อนบ้านหรือหนี้",
      totalMo: "รวมต่อเดือน",
      totalYr: "ต่อปี",
      totalAll: "ทั้งหมดที่ต้องมี",
      simpleMath: (mo: string, yr: string, yrs: number, tot: string) =>
        `${mo}/เดือน × 12 = ${yr}/ปี. ${yr} × ${yrs} ปี = ${tot}. นั่นคือตัวเลขของคุณ`,
    },
    zh: {
      overline: "你需要什么",
      h2: "退休要花多少钱？",
      body: "把它想成一张永不停止的月度账单。把每月必须花的钱加起来——然后乘以退休后的年数。",
      monthly: "退休后月支出",
      ikigaiLabel: "总共需要",
      ref: (g: GeoConfig) => `参考：${g.name}大多数人舒适退休需要约${fmtC(g.retireRef, "th")}`,
      ...
      totalAll: "退休全程所需",
      simpleMath: (mo: string, yr: string, yrs: number, tot: string) =>
        `${mo}/月 × 12个月 = ${yr}/年. ${yr} × ${yrs}年 = ${tot}. 这就是你的数字。`,
    },
  },
  salary: {
    en: {
      ...
      investL: "FREE TO SAVE / MONTH",
      savLabel: "IF YOU SAVE THIS MUCH EACH MONTH",
      noInv: "Nothing left to save. Reduce expenses or raise income.",
      shortBy: (v: string) => `You're short by ${v}`,
      surplus: (v: string) => `You'd have ${v} extra`,
      savingsRateL: "HOW MUCH DO YOU SAVE?",
      savingsRateSub: (pct: number, amt: string) => `You put away ${pct}% of your pay — that's ${amt} every month`,
      currentSavingsL: "WHAT YOU ALREADY HAVE",
      pileLabel: "YOUR SAVINGS PILE",
      pileSub: "This is how much money you will have piled up by retirement — if you only save, and never invest.",
      pileYear: (y: number, amt: string) => `Year ${y}: ${amt}`,
      earnLabel: "TOTAL YOU'LL EARN",
      earnSub: "Every paycheck, added up, until you retire",
    },
    th: {
      ...
      investL: "เงินที่ออมได้ต่อเดือน",
      savLabel: "ถ้าออมเท่านี้ทุกเดือน",
      noInv: "ไม่มีเงินออม ลดค่าใช้จ่ายหรือเพิ่มรายได้",
      shortBy: (v: string) => `ขาดอีก ${v}`,
      surplus: (v: string) => `เหลือ ${v}`,
      savingsRateL: "ออมเท่าไหร่?",
      savingsRateSub: (pct: number, amt: string) => `เก็บ ${pct}% ของรายได้ — คือ ${amt} ต่อเดือน`,
      currentSavingsL: "เงินที่มีอยู่แล้ว",
      pileLabel: "กองเงินออมของคุณ",
      pileSub: "นี่คือเงินที่คุณจะสะสมได้ถึงเกษียณ — ถ้าออมอย่างเดียว ไม่ลงทุน",
      pileYear: (y: number, amt: string) => `ปีที่ ${y}: ${amt}`,
      earnLabel: "รายได้รวมตลอดชีวิต",
      earnSub: "ทุกเดือนที่ทำงาน รวมกันจนถึงเกษียณ",
    },
    zh: {
      overline: "你拥有什么",
      h2: "你的职业生涯能产生多少？",
      ...
      investL: "每月可储蓄金额",
      savLabel: "如果你每月存这么多",
      noInv: "没有可储蓄余额。减少支出或增加收入。",
      shortBy: (v: string) => `还差 ${v}`,
      surplus: (v: string) => `多出 ${v}`,
      savingsRateL: "你存多少？",
      savingsRateSub: (pct: number, amt: string) => `你存下收入的${pct}%——即每月${amt}`,
      currentSavingsL: "你已有的储蓄",
      pileLabel: "你的储蓄堆",
      pileSub: "这就是你到退休时能攒下的钱——如果只储蓄，不投资。",
      pileYear: (y: number, amt: string) => `第${y}年: ${amt}`,
      earnLabel: "工作生涯总收入",
      earnSub: "每份薪水加起来，直到退休",
    },
  },
  gap: {
    en: {
  ...
  safetyMargin: {
    en: {
      overline: "DO YOU HAVE ENOUGH?",
      h2: "Your pile vs. what you need.",
      body: "If the two numbers are the same, you're not safe. Things go wrong. You want your pile to be BIGGER than your need.",
      pileL: "YOUR SAVINGS PILE",
      needL: "WHAT YOU NEED",
      gapL: "THE GAP",
      shortBy: (v: string) => `You're missing ${v}`,
      surplus: (v: string) => `You have ${v} extra`,
      safetyL: "SAFETY SCORE",
      bufferNote: "If your pile is 30% bigger than your need, you can handle surprises — illness, market crashes, helping family.",
      onlySave: "This is the MOST you can get by saving alone. To close the gap, you need to invest.",
      yearsLeft: (y: number) => `You have ${y} years to build your pile.`,
    },
    th: {
      overline: "พอหรือยัง?",
      h2: "กองเงินของคุณ vs ที่ต้องการ",
      body: "ถ้าสองตัวเลขเท่ากัน คุณยังไม่ปลอดภัย อะไรก็เกิดขึ้นได้ คุณต้องการให้กองเงินใหญ่กว่าที่ต้องการ",
      pileL: "กองเงินออมของคุณ",
      needL: "ที่ต้องการ",
      gapL: "ช่องว่าง",
      shortBy: (v: string) => `ขาดอีก ${v}`,
      surplus: (v: string) => `เหลือ ${v}`,
      safetyL: "คะแนนความปลอดภัย",
      bufferNote: "ถ้ากองเงินใหญ่กว่าที่ต้องการ 30% คุณรับมือเซอร์ไพรส์ได้ — ป่วย ตลาดพัง ช่วยครอบครัว",
      onlySave: "นี่คือมากที่สุดที่ออมอย่างเดียวทำได้ จะเติมช่องว่างต้องลงทุน",
      yearsLeft: (y: number) => `เหลือเวลา ${y} ปี ในการสร้างกองเงิน`,
    },
    zh: {
      overline: "够了吗？",
      h2: "你的储蓄堆 vs 你的需求",
      body: "如果两个数字一样，你并不安全。事情会出错。你需要你的储蓄堆比需求更大。",
      pileL: "你的储蓄堆",
      needL: "你需要的",
      gapL: "缺口",
      shortBy: (v: string) => `还差 ${v}`,
      surplus: (v: string) => `多出 ${v}`,
      safetyL: "安全分数",
      bufferNote: "如果储蓄堆比需求大30%，你就能应对意外——生病、市场崩盘、帮助家人。",
      onlySave: "这就是仅靠储蓄能得到的最大值。要填补缺口，你需要投资。",
      yearsLeft: (y: number) => `你还有${y}年时间来建立你的储蓄堆。`,
    },
  },
  allocator: {
    en: {
