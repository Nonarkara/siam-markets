...
const L: Record<LocKey, {
  ...
  age: string; salary: string; location: string; growth: string; savings: string; inflation: string;
  ...
  whyExplain: (fin: string, yrs: number, infl: string) => string;
  ...
  en: {
    ...
    age: "YOUR AGE", salary: "MONTHLY SALARY", location: "WHERE YOU LIVE", growth: "RAISE / YEAR", savings: "YOU SAVE", inflation: "INFLATION / YEAR",
    ...
    need: "YOU NEED",
    whyExplain: (fin, yrs, infl) => `At 60 your pay will be ≈${fin}/mo. People keep about 70% of that lifestyle. Over ~${yrs} years past 60, with prices rising ${infl}%/yr, that is the number.`,
    ...
    disclaimer: "Need rises with medium inflation · returns nominal · cycle rates illustrative, from Dalio (2018) · past ≠ future · not investment advice",
  },
  th: {
    tag: "แผน",
    ...
    age: "อายุของคุณ", salary: "เงินเดือน", location: "คุณอยู่ที่ไหน", growth: "ขึ้นเงินเดือน/ปี", savings: "คุณออม", inflation: "เงินเฟ้อ/ปี",
    ...
    need: "คุณต้องมี",
    whyExplain: (fin, yrs, infl) => `ตอนอายุ 60 เงินเดือนคุณจะ ≈${fin}/เดือน คนเรารักษาประมาณ 70% ของไลฟ์สไตล์นั้น ตลอด ~${yrs} ปีหลัง 60 โดยราคาขึ้น ${infl}%/ปี นั่นคือตัวเลข`,
    ...
    disclaimer: "เป้าโตตามเงินเฟ้อระดับกลาง · ผลตอบแทนตามตัวเลข · อัตราวัฏจักรเป็นตัวอย่าง จาก Dalio (2018) · อดีต ≠ อนาคต · ไม่ใช่คำแนะนำการลงทุน",
  },
  zh: {
    tag: "计划",
    title: "你的财务视野",
    ...
    age: "你的年龄", salary: "月薪", location: "你住在哪", growth: "年涨薪", savings: "你存", inflation: "年通胀",
    ...
    need: "你需要",
    whyExplain: (fin, yrs, infl) => `60岁时你的月薪约 ${fin}。人们保留约70%的生活水准。在60岁后约 ${yrs} 年里，物价每年涨 ${infl}%，这就是目标数字。`,
    ...
    disclaimer: "目标随中等通胀上升 · 收益为名义值 · 周期利率为示例，源自 Dalio (2018) · 历史 ≠ 未来 · 非投资建议",
  },
};
...
export default function PlanPage() {
  ...
  const [inflation, setInflation]       = useState(MED_INFLATION.th); // medium scenario
  ...
  const annualCost0     = finalSalary * 0.70 * 12;                          // first retirement year
  const need            = inflatedNeed(annualCost0, inflation, yearsPostRetire); // costs rise each year
    ...
    <div className="page page-enter" style={{ width: "100%", maxWidth: 920, margin: "0 auto", padding: "24px 16px 80px" }}>
      ...
      <Act tag={t.what} head={t.whatSub} sub={t.investLine(fmtC(monthly, geo), yearsToRetire)} delay={0.05}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
          ...
          <Slider label={t.inflation} value={inflation} min={0} max={0.08} step={0.005} onChange={setInflation} fmt={(v) => `${(v * 100).toFixed(1)}%`} accent="var(--bear)" />
        </div>
        {/* Geo picker */}
        <div style={{ marginTop: 12, padding: "16px 20px", background: "var(--bg-raised)", border: "1px solid var(--line)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(80px, 1fr))", gap: 4, marginTop: 8 }}>
            {(Object.entries(GEOS) as [GeoKey, typeof GEOS.th][]).map(([k, cfg]) => (
              <button key={k} onClick={() => { setGeo(k); setSalary(cfg.defaultSalary); setInflation(MED_INFLATION[k]); }} style={{
      ...
      {/* WHY */}
      <Act tag={t.why} head={t.whySub} sub={t.whyExplain(fmtC(Math.round(finalSalary), geo), yearsPostRetire, (inflation * 100).toFixed(1))} delay={0.05}>
        <div style={{ padding: "24px", background: "var(--bg-raised)", border: "1px solid var(--line)" }}>
